@echo off
chcp 65001 >nul
title AI Agent System - Fixed Start

echo.
echo ========================================
echo 🔧 AI Agent System Fixed Start
echo ========================================
echo.
...
REM 既存コンテナの停止
echo 🛑 既存のコンテナを停止中...
docker-compose -f docker-compose.fixed.yml down >nul 2>&1
...
echo 📊 コンテナ状態:
docker-compose -f docker-compose.fixed.yml ps 2>nul || docker-compose -f docker-compose.gpu.yml ps

echo.
echo 📋 Ollamaログ:
echo ========================================
if "%GPU_MODE%"=="GPU" (
    docker-compose -f docker-compose.gpu.yml logs ollama --tail=20
) else (
    docker-compose -f docker-compose.fixed.yml logs ollama --tail=20
)
...
echo.
echo 🔧 管理コマンド:
echo    ログ確認: docker-compose -f docker-compose.fixed.yml logs -f
echo    停止: docker-compose -f docker-compose.fixed.yml down
echo    再起動: docker-compose -f docker-compose.fixed.yml restart
echo.
echo 🐛 デバッグコマンド:

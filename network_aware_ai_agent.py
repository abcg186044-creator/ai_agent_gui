#!/usr/bin/env python3
"""
Network-Aware AI Agent - コンテナ間通信と外部アクセス対応版
"""
...
def render_network_status(ai_agent):
    """ネットワーク状態表示"""
    st.subheader("🌐 ネットワーク接続状態")
    ...
    with col1:
        ...
        if connection_status["working_url"]:
            st.success(f"✅ {connection_status['working_url']}")
        else:
            st.error("❌ 接続できません")
...
def render_smart_voice_interface(ai_agent):
    """スマート音声インターフェース"""
    st.header("🎤️ スマート音声入力システム - ネットワーク対応版")
    ...
    with col2:
        render_network_status(ai_agent)
...
def main():
    """メイン処理"""
    st.set_page_config(
        page_title="🌐 Network-Aware AI Agent",
        page_icon="🌐",
        layout="wide",
        initial_sidebar_state="expanded"
    )
    
    st.title("🌐 Network-Aware AI Agent")
    st.markdown("### コンテナ間通信と外部アクセス対応版 - スマート音声入力システム")
